...
extern FILE *bestSolutionsOverIterationDVTC, *optimalSolutionFileDVTC;
...
void generatePopulationDVTC(partitionDVTC* populationDVTC, int indiceFirstElt)
{
    ...
    for(i=indiceFirstElt; i<taillePopulation; i++)
    {
        for(j=0; j<nbrNoeuds; j++)
        {
            ...
            (populationDVTC+i)->genotype[j] = rnd(0,max_clusters);
...
void checkContrainstAndFitnessPenalizationDVTC(partitionDVTC *populationDVTC)
{
    ...
    for(i=0; i<taillePopulation; i++)
    {
        ...
        if((populationDVTC+i)->nbrCluster > max_clusters || (populationDVTC+i)->nbrCluster < min_clusters)
        {
            (populationDVTC+i)->constraintVector[0] += 1; /// le nombre de cluster n'est pas valide
        }
        ...
        for(j=0; j < (populationDVTC+i)->nbrCluster; j++)
        {
            if((populationDVTC+i)->clusters[j].clusterSize  > max_sizeCluster ||
                (populationDVTC+i)->clusters[j].clusterSize  < min_sizeCluster)
            {
                (populationDVTC+i)->constraintVector[1] += 1;
            }
        }
        if((populationDVTC+i)->constraintVector[1] != 0)
        {
            (populationDVTC+i)->contrainteViole++;
        }
    }
...
void checkCohabitationAndNonCohabitationConstrainteDVTC(partitionDVTC *populationDVTC)
{
    ...
    for(i=0; i<taillePopulation; i++)
    {
        /// This task is already done by the  checkCohabitationAndNonCohabitationConstrainteBE function
		/**
		...
		*/
        ...
        if(nbrNonCohabitationConstraintes!=0)
        {
            ...
            /// cette instruction permet d'avoir le nombre de contraintes violees
...
///********************************************************************************************
void writeOptimalSolutionInFileDVTC(partitionDVTC *solutionDominante,FILE* optimalSolutionFileDVTC,
                                  int nbrRun, metrics metricsDVTC)
{
    int i,j,k;
    fprintf(optimalSolutionFileDVTC,"%d,",nbrRun);
    fprintf(optimalSolutionFileDVTC,"%d,%d,%d,%.3f,%.3f,",
        solutionDominante->coutCoupe,
        (sommeTotalFlux - solutionDominante->coutCoupe),
        sommeTotalFlux ,
        metricsDVTC.runTime,
        metricsDVTC.ES);
    fprintf(optimalSolutionFileDVTC,"%d,%d,%d,",
        nbrNoeuds,
        nbrArretes,
        solutionDominante->contrainteViole);
    ...
    ///iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii
    fprintf(optimalSolutionFileDVTC,"%d,%d,%d,%d,",
    ...
    for(j=0; j<solutionDominante->nbrCluster; j++){
        countNoued = countNoued + solutionDominante->clusters[j].clusterSize;
        // printf("\nclusters[%d].clusterSize = %d\n", j, solutionDominante->clusters[j].clusterSize);
    }
    ...
    */
    ...
    for(j=0; j < solutionDominante->nbrCluster ; j++)
    {
        fprintf(optimalSolutionFileDVTC,"{");
        for(k=0;k< solutionDominante->clusters[j].clusterSize; k++){
            fprintf(optimalSolutionFileDVTC,"%d ",solutionDominante->clusters[j].clusterNoueds[k]);
        }
        if(j <= solutionDominante->nbrCluster - 1)fprintf(optimalSolutionFileDVTC,"}\t");
    }
    fprintf(optimalSolutionFileDVTC,"\n");
}
